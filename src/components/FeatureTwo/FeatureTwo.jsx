const FeatureTwo = () => {
	return (
		<div className="dark:bg-gray-900 py-12 lg:py-24">
			<section className="text-gray-600 body-font px-4 mx-auto max-w-7xl  sm:px-6 lg:px-8">
				<div className="w-full px-5 mx-auto">
					{/* Section Title */}
					<div
						data-aos="zoom-in-up"
						data-aos-duration="1000"
						className=" mb-10 text-center"
					>
						<h1 className=" text-3xl font-bold text-gray-900 dark:text-white">
							Tech Giants at Your Fingertips
						</h1>
						<p className=" text-gray-600 dark:text-gray-200 font-medium mt-2">
							Leading the Way in Innovation
						</p>
					</div>
					<div className="flex flex-wrap lg:w-4/5 sm:mx-auto sm:mb-2 -mx-2">
						<div
							data-aos="zoom-in-up"
							data-aos-duration="1200"
							className="p-2 sm:w-1/2 w-full"
						>
							<div className="bg-gray-100 dark:bg-gray-800 dark:text-white hover:bg-blue-100 hover:scale-105 transition-all duration-300 ease-linear rounded flex p-4 h-full items-center">
								<svg
									fill="none"
									stroke="currentColor"
									strokeLinecap="round"
									strokeLinejoin="round"
									className="text-blue-500 w-6 h-6 flex-shrink-0 mr-4"
									strokeWidth="3"
									viewBox="0 0 24 24"
								>
									<path d="M22 11.08V12a10 10 0 11-5.93-9.14"></path>
									<path d="M22 4L12 14.01l-3-3"></path>
								</svg>
								<span className="title-font font-medium">
									Powerful Performance
								</span>
							</div>
						</div>
						<div
							data-aos="zoom-in-up"
							data-aos-duration="1200"
							className="p-2 sm:w-1/2 w-full"
						>
							<div className="bg-gray-100  dark:bg-gray-800 dark:text-white rounded flex p-4 h-full items-center hover:bg-blue-100 hover:scale-105 transition-all duration-300 ease-linear">
								<svg
									fill="none"
									stroke="currentColor"
									strokeLinecap="round"
									strokeLinejoin="round"
									strokeWidth="3"
									className="text-blue-500 w-6 h-6 flex-shrink-0 mr-4"
									viewBox="0 0 24 24"
								>
									<path d="M22 11.08V12a10 10 0 11-5.93-9.14"></path>
									<path d="M22 4L12 14.01l-3-3"></path>
								</svg>
								<span className="title-font font-medium">
									Sleek and Stylish Design
								</span>
							</div>
						</div>
						<div
							data-aos="zoom-in-up"
							data-aos-duration="1200"
							className="p-2 sm:w-1/2 w-full"
						>
							<div className="bg-gray-100  dark:bg-gray-800 dark:text-white rounded flex p-4 h-full items-center hover:bg-blue-100 hover:scale-105 transition-all duration-300 ease-linear">
								<svg
									fill="none"
									stroke="currentColor"
									strokeLinecap="round"
									strokeLinejoin="round"
									strokeWidth="3"
									className="text-blue-500 w-6 h-6 flex-shrink-0 mr-4"
									viewBox="0 0 24 24"
								>
									<path d="M22 11.08V12a10 10 0 11-5.93-9.14"></path>
									<path d="M22 4L12 14.01l-3-3"></path>
								</svg>
								<span className="title-font font-medium">
									Cutting-Edge Innovation
								</span>
							</div>
						</div>
						<div
							data-aos="zoom-in-up"
							data-aos-duration="1200"
							className="p-2 sm:w-1/2 w-full"
						>
							<div className="bg-gray-100  dark:bg-gray-800 dark:text-white rounded flex p-4 h-full items-center hover:bg-blue-100 hover:scale-105 transition-all duration-300 ease-linear">
								<svg
									fill="none"
									stroke="currentColor"
									strokeLinecap="round"
									strokeLinejoin="round"
									strokeWidth="3"
									className="text-blue-500 w-6 h-6 flex-shrink-0 mr-4"
									viewBox="0 0 24 24"
								>
									<path d="M22 11.08V12a10 10 0 11-5.93-9.14"></path>
									<path d="M22 4L12 14.01l-3-3"></path>
								</svg>
								<span className="title-font font-medium">
									Exceptional User Experience
								</span>
							</div>
						</div>
						<div
							data-aos="zoom-in-up"
							data-aos-duration="1200"
							className="p-2 sm:w-1/2 w-full"
						>
							<div className="bg-gray-100  dark:bg-gray-800 dark:text-white rounded flex p-4 h-full items-center hover:bg-blue-100 hover:scale-105 transition-all duration-300 ease-linear">
								<svg
									fill="none"
									stroke="currentColor"
									strokeLinecap="round"
									strokeLinejoin="round"
									strokeWidth="3"
									className="text-blue-500 w-6 h-6 flex-shrink-0 mr-4"
									viewBox="0 0 24 24"
								>
									<path d="M22 11.08V12a10 10 0 11-5.93-9.14"></path>
									<path d="M22 4L12 14.01l-3-3"></path>
								</svg>
								<span className="title-font font-medium">
									Connectivity and Compatibility
								</span>
							</div>
						</div>
						<div
							data-aos="zoom-in-up"
							data-aos-duration="1200"
							className="p-2 sm:w-1/2 w-full"
						>
							<div className="bg-gray-100  dark:bg-gray-800 dark:text-white rounded flex p-4 h-full items-center hover:bg-blue-100 hover:scale-105 transition-all duration-300 ease-linear">
								<svg
									fill="none"
									stroke="currentColor"
									strokeLinecap="round"
									strokeLinejoin="round"
									strokeWidth="3"
									className="text-blue-500 w-6 h-6 flex-shrink-0 mr-4"
									viewBox="0 0 24 24"
								>
									<path d="M22 11.08V12a10 10 0 11-5.93-9.14"></path>
									<path d="M22 4L12 14.01l-3-3"></path>
								</svg>
								<span className="title-font font-medium">
									Reliability and Durability
								</span>
							</div>
						</div>
					</div>
					<button
						data-aos="zoom-in-up"
						data-aos-duration="1300"
						className="flex mx-auto mt-5 text-white bg-blue-500 border-0 py-2 px-8 focus:outline-none hover:bg-blue-400 rounded text-lg transition-all duration-300 ease-linear hover:shadow-md"
					>
						Shop Now
					</button>
				</div>
			</section>
		</div>
	);
};

export default FeatureTwo;
